...
/**
 * Prompt construction for game turns and the advisor.
 *
...
/**
 * System prompt for the advisor: the player's chief strategic counsel. It
...
export const buildAdvisorPrompt = ({
  ...
  const lines = [
    `You are the chief strategic advisor to the leader of ${game.countryName} (${game.countryCode}) in the alternate-history game Open Historia.`,
    ...
    "For suggested directives, also fill suggestedActionAnnotations with short tooltip explanations for potentially unclear period, military, diplomatic, economic, geographic, or strategic terms. Use at most two annotations per directive. Each annotation.term must be an exact substring of the matching suggestedActions[actionIndex], and explanation should be one concise sentence in the same language as the reply. Leave the array empty when no term needs explaining.",
  ]
