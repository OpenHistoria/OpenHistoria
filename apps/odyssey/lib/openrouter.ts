 ...
 * openrouter.ai with an S256 code challenge, they authorize Open Odyssey, and
 * we exchange the returned code for an app-scoped API key. That key is stored
 * in this browser only and is never sent to our servers; players can cap or
 * revoke it anytime from their OpenRouter dashboard.
 */

const KEY_STORAGE_KEY = "openodyssey:openrouter-key"
const VERIFIER_STORAGE_KEY = "openodyssey:openrouter-verifier"
const PREFER_FREE_ROTATION_KEY = "openodyssey:prefer-free-rotation"

/** Fired on window whenever the stored key changes in this tab. */
export const OPENROUTER_KEY_CHANGED_EVENT = "openodyssey:openrouter-key-changed"
...
/**
 * Whether new adventures should default to the free-model rotation. Set when a
 * key is injected directly (the CI screenshot agent), so an automated run
 * plays on free models without touching any model picker.
 */
 ...
 * stores it, switches new adventures to the free-model rotation, and scrubs
 * the key from the address bar. Returns true when a key was ingested.
 */
...
/**
 * Fetches metadata about the stored key. Resolves to Ok(null) when the key has
 * been revoked or is otherwise invalid (the caller should treat that as
 * disconnected).
 */
