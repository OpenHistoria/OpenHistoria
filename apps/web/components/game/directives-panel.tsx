...
import { InlineExplainedText } from "@/components/game/inline-explained-text"
...
export function DirectivesPanel({
  ...
  return (
    ...
    >
      ...
      {directiveOutcomes.length > 0 && (
        <section className="mt-4">
          <h3 className="mb-1.5 text-[11px] font-semibold tracking-wide text-muted-foreground uppercase">
            {t.game.directivesOutcomeTitle}
          </h3>
          <ul className="flex flex-col gap-2">
            {directiveOutcomes.map((outcome, index) => (
              <li
                key={`${index}-${outcome.text}`}
              ...
              >
                <div className="mb-1.5 flex items-center justify-between gap-2">
                  ...
                  >
                    {outcome.status === "advanced"
                      ? t.game.directivesOutcomeAdvanced
                      : t.game.directivesOutcomeUnclear}
                  </span>
                </div>
                <p className="text-xs leading-snug text-muted-foreground">
                  {outcome.summary || t.game.directivesOutcomeFallback}
                </p>
              </li>
            ))}
          </ul>
      ...
      {directives.length === 0 ? (
        ...
        <ul className="mt-4 flex flex-col gap-2">
          {directives.map((directive, index) => (
            ...
            >
              <span className="min-w-0 flex-1 text-sm break-words whitespace-pre-wrap text-foreground/90">
                <InlineExplainedText
                  text={directive}
                  annotations={directiveAnnotations[index] ?? []}
                />
              </span>
              <button
                type="button"
