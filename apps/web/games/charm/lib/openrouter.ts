 ...
 * openrouter.ai with an S256 code challenge, they authorize Open Charm, and we
 * exchange the returned code for an app-scoped API key. That key is stored in
 * this browser only and is never sent to our servers; players can cap or revoke
 * it anytime from their OpenRouter dashboard.
 */
...
/**
 * Whether new flirtations should default to the free-model rotation. Set when a
 * key is injected directly (the CI screenshot agent), so an automated run plays
 * on free models without touching any model picker.
 */
