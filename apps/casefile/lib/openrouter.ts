 ...
 * openrouter.ai with an S256 code challenge, they authorize Open Case, and we
 * exchange the returned code for an app-scoped API key. That key is stored in
 * this browser only and is never sent to our servers; players can cap or
 * revoke it anytime from their OpenRouter dashboard.
 */

const KEY_STORAGE_KEY = "opencase:openrouter-key"
const VERIFIER_STORAGE_KEY = "opencase:openrouter-verifier"
const PREFER_FREE_ROTATION_KEY = "opencase:prefer-free-rotation"

/** Fired on window whenever the stored key changes in this tab. */
export const OPENROUTER_KEY_CHANGED_EVENT = "opencase:openrouter-key-changed"
...
/**
 * Whether new cases should default to the free-model rotation. Set when a key
 * is injected directly (the CI screenshot agent), so an automated run plays on
 * free models without touching any model picker.
 */
 ...
 * stores it, switches new cases to the free-model rotation, and scrubs the key
 * from the address bar. Returns true when a key was ingested.
 */
