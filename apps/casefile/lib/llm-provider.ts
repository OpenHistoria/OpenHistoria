...
/**
 * Local provider override for the detective engine.
 *
 * By default Open Case drives the investigation through OpenRouter with the
 * player's own key. For local or self-hosted setups you can instead point the
 * engine at any OpenAI-compatible chat-completions endpoint - Ollama, a
 * LiteLLM proxy, vLLM, LM Studio, etc. - and bypass OpenRouter entirely.
 *
 * Configure via env (baked at build time) or localStorage (read at runtime and
 * taking precedence):
 *
 *   base URL  NEXT_PUBLIC_LLM_BASE_URL  / localStorage "opencase:llm-base-url"
 *   api key   NEXT_PUBLIC_LLM_API_KEY   / localStorage "opencase:llm-api-key"
 *   model     NEXT_PUBLIC_LLM_MODEL     / localStorage "opencase:llm-model"
 */

export const LLM_BASE_URL_STORAGE_KEY = "opencase:llm-base-url"
export const LLM_API_KEY_STORAGE_KEY = "opencase:llm-api-key"
export const LLM_MODEL_STORAGE_KEY = "opencase:llm-model"
...
/** Model id new cases default to when a local provider is configured. */
